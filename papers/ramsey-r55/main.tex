...
\hypersetup{
  ...
  pdftitle={Computational Lower Bounds for R(5,5): A Constructive Proof
...
\begin{center}
  {\fontsize{22}{26}\selectfont\bfseries\color{accent}
   Computational Lower Bounds for $R(5,5)$}\\[6pt]
  ...
  {\small\color{subtlegray}\itshape v1.0\enspace---\enspace
    Exhaustive Verification at 962{,}598 Five-Cliques}
\end{center}
...
\begin{definition}
The \emph{Ramsey number} $R(s,t)$ is the minimum $n \in \N$ such
...
\end{axis}
\end{tikzpicture}
\caption{Raw violation counts on $K_{43}$ ($k=5$). The Paley
  construction produces 1{,}380 violations; the best cycle-type
  seed from $\GF(43)$ achieves only 10 --- a $138\times$ improvement.}
...
\begin{proof}
We exhibit the coloring stored in the certificate file
\texttt{ramsey\_checkpoint\_K42.json}. The coloring was discovered
...
\end{enumerate}
Verification: an independent brute-force check iterates over all
$\binom{42}{5} = 850{,}668$ five-cliques and confirms that no
subset of five vertices has all $\binom{5}{2} = 10$ edges in the
same color.
\end{proof}
...
overcome an energy barrier to escape this configuration. The
barrier height (minimum number of simultaneous flips required to
reach a lower-violation state) is at least 2, as confirmed by
extensive multi-flip perturbation experiments.
\end{remark}
...
\emph{critical temperature} $T_c$, estimated as
\[
...
\begin{axis}[
  ...
  symbolic y coords={
    ...
    {GPU ILS (solve\_ils)},
    {Wider prime sweep},
    {Ising router (12 solvers)},
    {Vertex destruction},
    {Pop. crossover (50 gen)},
    {Targeted perturbation},
    {$\mathbb{Z}_2$ complement},
    {Temp. ladder (8 $T_c$)},
    {CPU ILS ultra (20$\times$500)},
    {GPU SA (50 $T_c$ sweeps)}
  },
  ytick=data,
...
\addplot[fill=accent!50, draw=accent] coordinates {
  (2593, {GPU SA (50 $T_c$ sweeps)})
  (12931, {CPU ILS ultra (20$\times$500)})
  (2300, {Temp. ladder (8 $T_c$)})
  (3787, {$\mathbb{Z}_2$ complement})
  (12314, {Targeted perturbation})
  (12150, {Pop. crossover (50 gen)})
  (527, {Vertex destruction})
  (29, {Ising router (12 solvers)})
  (4752, {Wider prime sweep})
  (8515, {GPU ILS (solve\_ils)})
...
The two-violation floor is not an artifact of $\GF(43)$ seeding.
Independent cycle-type seeds from $\GF(83)$ ($b=40$, $c=48$, $d=1$,
raw violations $= 110$) also converge to two violations after ILS.
...
generations with block sizes 25--35, suggesting the constraint
...
Electron. J. Combin., Dynamic Surveys DS1, 2021.
